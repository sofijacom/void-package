...
LATEST_VERSION=$(gh release list --repo KDE/smb4k --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
export VERSION=${LATEST_VERSION#"smb4k-"}
...
gh release download -R KDE/smb4k --archive=tar.gz --output "${VERSION}.tar.gz"
...
printf "KDE/smb4k template updated\n"
