...
LATEST_VERSION=$(curl -Ls -o /dev/null -w '%{url_effective}' https://repo.yandex.ru/yandex-browser/deb/pool/main/y/yandex-browser-stable/ --exclude-pre-releases --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
