...
LATEST_VERSION=$(gh release list --repo nwg-piotr/nwg-look --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R nwg-piotr/nwg-look --archive=tar.gz --output "${VERSION}.tar.gz"
