...
LATEST_VERSION=$(curl -Ss --request GET "https://repo.yandex.ru/yandex-browser/deb/pool/main/y/yandex-browser-stable/*deb" | jq -r '.[0] | .name')
...
export SHA256=$(curl -L https://repo.yandex.ru/yandex-browser/deb/pool/main/y/${LATEST_VERSION}/yandex-browser-stable_${VERSION}_amd64.deb.sha256 | cut -d ' ' -f1 )
