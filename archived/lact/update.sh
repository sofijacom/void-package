...
LATEST_VERSION=$(gh release list --repo ilya-zlobintsev/LACT --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R ilya-zlobintsev/LACT --archive=tar.gz --output "v${VERSION}.tar.gz"
