...
LATEST_VERSION=$(curl -Ss --request GET https://repo.yandex.ru/yandex-browser/deb/pool/main/y/yandex-browser-stable/ | jq -r .tag_name | sed "s/^v//")
...
export SHA256=$(curl -L -o https://repo.yandex.ru/yandex-browser/deb/pool/main/y/yandex-browser-stable/v${VERSION}/yandex-browser-stable_${VERSION}_amd64.deb.sha256 | cut -d ' ' -f1 )
