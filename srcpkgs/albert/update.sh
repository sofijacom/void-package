...
LATEST_VERSION=$(gh release list --repo albertlauncher/albert --json name,tagName,isLatest --jq '.[] | select(.isLatest)|.tagName')
...
gh release download -R albertlauncher/albert --archive=tar.gz --output "v${VERSION}.tar.gz"
...
printf "albert template updated\n"
